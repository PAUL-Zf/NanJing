...
const actions = {
  ...
  fetchAll({ commit }) {
    let st = new Date();
    dataService.fetchAll({ commit }, (resp) => {
      //获取到后端站点数据之后,传到resp,resp就是一个数组或者json数据
  ...
  fetchAllData({ commit }) {
    let st = new Date();
    dataService.fetchAllData({ commit }, (resp) => {
      //获取到后端站点数据之后,传到resp,resp就是一个数组或者json数据
...
const mutations = {
  ...
  selectPie(state, data) {
    if (state.selectPie == data) {
      state.selectPie = undefined;
    } else {
      state.selectPie = data;
    }
  },
